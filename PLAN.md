...
**详细流程：**
1. 轮到当前玩家 → 系统**随机选择**一个未摧毁方格作为默认发射点，用户也可以点击切换；更新可用点数显示
...
6. 存档管理页点击 LOAD → 从 JSON 读回游戏状态，回到游戏页，从存档回合继续
7. 存档管理页点击 DELETE → 删除对应 JSON 文件
8. 游戏过程中：
   - 点击方格切换发射点（Canvas 重绘高亮）
   - 输入区输入表达式 → 实时计算消耗点数显示
...
| 9 | 点数系统 | AST 遍历计费 + 每轮点数递增公式 + 输入框实时消耗显示 |
| 10 | 障碍物碰撞检测 | 轨迹点优先检测障碍物；碰到立即中止并破坏障碍物 |
| 11 | 对方方块碰撞检测 | 障碍物之后检查对方方块区域判定 |
| 12 | ViewModel + 状态机 | GameViewModel 完整游戏流程、随机发射点、玩家点击选择发射点 |
