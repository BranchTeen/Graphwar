...
> 这次项目让我深刻体会到"架构"在实际工程中的意义。从 EventBus 全局单例到 PropertyTrigger + Command 模式的重构，让每个 View 组件只知道自己需要的命令和数据，不再依赖全局通信总线。这种"减少隐式依赖"的思路对我理解软件设计有质的帮助。
>
> 另一个收获是 AI 协作中的"渐进式需求"。人类不需要一开始就提出完美需求——可以先说"保存后页面没刷新"，再补充"删除存档不该跳转到游戏页"，再进一步"View 不应该包含 Model 头文件"。AI 能够在每一轮迭代中理解上下文并逐步逼近最终结果。这种渐进式协作方式比传统的"一次写完所有需求再让 AI 实现"要高效得多。
